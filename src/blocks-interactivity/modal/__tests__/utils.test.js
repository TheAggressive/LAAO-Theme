...
describe( 'Debug utility', () => {
	...
	it( 'stores logs in the logs array', () => {
		...
		expect( Debug.getLogs().some( ( l ) => l.message.includes( 'stored message' ) ) ).toBe( true );
		spy.mockRestore();
	} );
...
describe( 'isElementVisible', () => {
	...
	it( 'returns true when element is fully within viewport', () => {
		const el = {
			getBoundingClientRect: () => ( { top: 10, left: 10, bottom: 100, right: 200 } ),
		};
	...
	it( 'returns false when element is below the viewport', () => {
		const el = {
			getBoundingClientRect: () => ( { top: 900, left: 10, bottom: 1000, right: 200 } ),
		};
		Object.defineProperty( window, 'innerHeight', { value: 800, configurable: true } );
