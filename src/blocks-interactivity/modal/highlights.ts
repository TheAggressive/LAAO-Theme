...
const getEditorDocuments = (): Document[] => {
	...
	for (const selector of EDITOR_IFRAME_SELECTORS) {
		document
			.querySelectorAll<HTMLIFrameElement>(selector)
			.forEach((iframe) => {
				try {
...
export const findBlockDomElement = (clientId: string): Element | null => {
	...
	for (const doc of getEditorDocuments()) {
		for (const selector of alternativeSelectors) {
			...
			if (blockElement && isInEditorCanvas(blockElement)) {
				if (doc !== document) {
					Debug.add(
						`Found block ${clientId} in editor canvas document`
					);
				}
				return blockElement;
			}
	...
	try {
		...
		Debug.add(
			`Error accessing block store: ${(error as Error).message}`,
			true
		);
	}
...
export const cleanupAllHighlights = (modalId: string | null = null): void => {
	...
	for (const doc of getEditorDocuments()) {
		doc.querySelectorAll(
			'style[id^="modal-direct-highlight-style-"]'
		).forEach((styleTag) => {
	...
	try {
		...
		Debug.add(
			`Error in final cleanup pass: ${(error as Error).message}`,
			true
		);
	}
};
...
export const findTriggerElement = (
	...
	if (potentialTriggers.length === 1) {
		return potentialTriggers[0] ?? null;
	}
