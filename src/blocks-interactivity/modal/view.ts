...
function getShell(id: string): HTMLElement | null {
	return (
		(Array.from(
			document.getElementsByClassName('wp-block-laao-modal__shell')
...
function bindExternalTrigger(
	...
	el.setAttribute(
		'aria-expanded',
		modalsState[id]?.isOpen ? 'true' : 'false'
	);
...
function openModal(id: string, modalsState: Record<string, ModalState>): void {
	...
	document.dispatchEvent(
		new CustomEvent('aa:modal:open', { detail: { id } })
	);
		...
		requestAnimationFrame(() => {
			if (modalsState[id]?.isOpen) {
				announcer.textContent =
					announcer.dataset.label ?? 'Dialog opened';
			}
		});
	}
...
function closeModal(id: string, modalsState: Record<string, ModalState>): void {
	...
	document.dispatchEvent(
		new CustomEvent('aa:modal:close', { detail: { id } })
	);
	...
	const effectiveDuration =
		reducedMotion || exitName === 'none' ? 0 : duration;
		...
		init(): void {
			...
			if (state.modals[id].openOnLoad) {
				if (
					!shouldSkipOpenOnLoadOnce(
						id,
						state.modals[id].openOnLoadOnce
					)
				) {
